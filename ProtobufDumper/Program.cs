        ...
        private static void Main(string[] args)
        {
            ...
            for (var i = 0; i < unnamedArgs.Count; i++)
            {
                ...
                if (i == 0 || i < unnamedArgs.Count - 1)
                {
                    ...
                    Console.WriteLine($"Could not find file {unnamedArgs[i]}");
                ...
                Console.WriteLine($"Output directory path is not valid {unnamedArgs[i]}");
            ...
            {
                Console.WriteLine($"Loading binary '{target}'...");
                ...
                ExecutableScanner.ScanFile(target, (name, buffer) =>
                {
                    ...
                    switch (result)
                    {
                        ...
                        case ProtobufCollector.CandidateResult.Rescan:
                            ...
                            Console.WriteLine($"needs rescan: {error.Message}");
                            Console.ResetColor();
                            break;

                        default:
                            Console.ForegroundColor = ConsoleColor.Red;
                            Console.WriteLine($"is invalid: {error.Message}");
                            Console.ResetColor();
                            break;
                    }
                    ...
                    if (complete && hasDumpCandidates)
                    {
                        ...
                        {
                            Console.WriteLine($"Unable to dump: {ex.Message}");
                        }
                    }
            ...
            if (dumper.Analyze())
            {
                dumper.DumpFiles((name, buffer) =>
                {
                    ...
                    Console.WriteLine($"  ! Outputting proto to '{outputFile}'");
